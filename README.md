...
- **Нет демо-анкет после запуска.** Посмотрите в `logs/matchly.log` строки `Demo data`: там указано, включена ли генерация (`MATCHLY_SEED`) и почему она пропущена. Создать анкеты можно в любой момент: войдите администратором, откройте «Админ» → «Настройки» → «Создать демо-анкеты» (или `POST /api/admin/demo-data`).
