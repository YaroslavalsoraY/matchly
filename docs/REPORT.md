...
| Тестируемость | 89 автотестов: 36 unit-тестов (сервисы с Mockito, доменные правила анкеты, алгоритмы стратегий на заранее известных матрицах) и 53 API-теста (MockMvc, полный контекст Spring на H2), включая граничные значения и некорректные параметры. Плюс два браузерных сценария Playwright: основной путь (18 шагов) и корнер-кейсы (19 групп). |
...
| API (MockMvc, контекст Spring, H2) | регистрация и вход, формат ошибок, права доступа, блокировка с мгновенным отзывом токена, каскадное удаление, анкеты и фото (в том числе пустой и слишком большой файл), справочник, реакции и матчи, рекомендации и настройки, статистика, генератор демо-данных, граничные значения (18/99 лет, ровно 10 интересов, лимиты пагинации, нечисловые параметры, неизвестные адреса и методы) | 53 |
| Сквозные (Playwright, Chromium) | основной путь: 18 шагов от регистрации до админки; корнер-кейсы: 19 групп (маршруты без сессии, испорченный токен, переводы ошибок, глубокие ссылки без анкеты, валидация с подсветкой полей, лимит интересов, провал загрузки фото, пустая лента, разметка в имени и описании, сохранение выбранного алгоритма, двойной клик, горячие клавиши при открытом окне, блокировка во время работы, удаление аккаунта, пагинация и поиск в админке, отсутствие горизонтальной прокрутки на 400px для всех экранов) | 2 сценария |

Дополнительно каждый этап проверялся вживую на PostgreSQL 16: старт приложения, миграции, запросы через `curl`,
содержимое таблиц, ответ 413 на файл больше 2 МБ.

Браузерные тесты нашли и помогли исправить три реальных дефекта: пустая лента у части демо-пользователей
(генератор «просматривал» за них всех кандидатов), локализация сообщений валидации по заголовку браузера
(теперь контракт API фиксирован на английском, интерфейс переводит сам) и выход из аккаунта при неверном пароле
подтверждения удаления (сервер отвечал 401, теперь 422).

Запуск: `./mvnw test` (Windows: `mvnw.cmd test`). Тесты не требуют PostgreSQL и Docker.

## 8. Запуск и интеграция

Единая точка входа: `start.sh` (Linux, macOS) и `start.cmd` (Windows). Скрипт проверяет Java 21, находит PostgreSQL
или поднимает его в Docker, затем запускает приложение с интерфейсом. Режим `--docker` собирает образ по многоэтапному
`Dockerfile` и запускает базу и приложение через `docker-compose.yml`; Java на машине при этом не нужна.
Настройки задаются файлом `.env`. Подробности для Windows и Linux: [README.md](../README.md).
